...
from recognizeapp.utils import (
    ...
    dedupe_images,
    encode_faces,
    generate_html_report, FindingType, EncodingType,
    get_chunks,
)
...
def notify_status(counter: int, filepath: str, task: Task, size: int, **kwargs):
    ...
    signals.task_prerun.send(sender=task.name, task_id=task.request.id, custom_state=custom_state)
...
@app.task(bind=True, base=DedupeTask)
def callback_findings(self: Task, results: list[FindingType], config: dict[str, Any]) -> dict[str, Any]:
...
@app.task(bind=True, base=DedupeTask)
def callback_encodings(self: Task, results: tuple[EncodingType, int, int], config: Dict[str, Any]) -> Dict[str, Any]:
...
def save_report(self: Task, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ds = Dataset(config)
    content, __ = generate_html_report(ds.get_findings(), {})
    report_file = Path(ds._get_filename("report", ".html"))
...
def deduplicate_dataset(self: Task, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    size = len(chunks)
    #
    tasks = [dedupe_chunk.s(chunk, f"{n}/{size}", config) for n, chunk in enumerate(chunks)]
...
def process_dataset(self: Task, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    size = len(chunks)
    tasks = [encode_chunk.s(chunk, f"{n}/{size}", config) for n, chunk in enumerate(chunks)]
